...
import { AsyncResult, asyncResultify, createAsyncResult } from './asyncResult';
...
describe('Result methods', () => {
  ...
  it('composite test showing all simple result getters', async () => {
    ...
    expect(
      await asyncErrRes.unwrapOrElse(() => {
        return 2;
      }),
    ).toEqual(2);

    expect(
      await asyncErrRes.unwrapOrElse(async () => {
        return 2;
      }),
    ).toEqual(2);
...
describe('Utils', () => {
  ...
  it('asyncResultify Err result - calling new function returns AsyncResult and errMapper gets applied to result', async () => {
    const test = asyncResultify(
      async () => {
        throw 1;
      },
  ...
  it('createAsyncResult fn correctly creates function returning AsyncResult from fn returning ResultPromise', async () => {
    const test = createAsyncResult(async (num: number) => {
      if (num >= 0) {
        return Ok(num);
      }
      return Err('err');
